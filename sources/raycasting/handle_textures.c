...
/*   Updated: 2024/09/12 23:07:53 by luifer           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_update_texture(t_data *data, t_textinfo *texture, t_ray *ray, int x)
{
	...
	texture->position = (ray->draw_start - data->window_height / 2 + ray->line_height / 2) * texture->step;
	y = ray->draw_start;
	while (y < ray->draw_end)
	{
		texture->x = (int)texture->position & (texture->size - 1);
		texture->position += texture->step;
		color = data->textures[texture->idx][texture->size * texture->y + texture->x];
